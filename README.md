...
## 22.7.08 (slf4j, log4j, logback 차이 추가)
# JAVA
...
빌드 테스트 배포 프로세르 자동화 -> 소프트웨어 품질개선 개발생산성 증가
...
* log4j
가장 오래된 로깅 프레임워크로 apache의 java 기반 로깅 프레임워크. 콘솔 및 <b>파일출력</b>의 형태로 로깅을 도와주며
xml, properties로 환경을 구성할 수 있다. <b>2015기준 개발중단</b>
* logback
log4j 이후에 출시된 보다 향상되고 널리 사용되고 있는 JAVA 로깅 프레임워크중 하나이다. slf4j의 구현체로써 스프링부트의 경우
기본적으로 logback이 포함되어있어 별도의 dependency 추가없이 사용할 수 있다.
log4j에 비해 향상된 필터링 정책 및 기능(ex. 로그인 한 경우 logging 등등)과 로그 레벨 변경등에 대해 서버 재시작 없는 자동 리로딩 지원
* log4j2
가장 최신에 나온 로깅 프레임워크로 logback 처럼 필터링 기능과 자동 리로딩을 지원한다. logback과 차이는 Multi Thread 환경에서 비동기
로거(Async Logger)의 경우 다른 로깅 프레임워크보다 처리량이 훨씬 많고, 대기 시간이 훨씬 짧다. 또한 람다식을 지원하고 Lazy Evalutaion을 지원한다
* slf4j
Logging 관련 라이브러리들을 하나의 통일된 방식으로 사용할 수 있는 방법을 제공하는 interface 모음.
로깅 라이브러리를 어떤 것을 사용하던지 같은 방법으로 로그를 남길 수 있게함.(로그 라이브러리를 교체하더라도, 어플리케이션의 코드는 변경될 필요가 없음.)
...
## mvc1 / mvc2 차이점
* mvc1 : 뷰와 컨트롤러의 역할이 합쳐져 있다.에 jsp에 자바코드와 html css 등의 코드가 섞여있어
소스가 복잡해져 유지보수가 힘들어 진다.
하지만 상대적으로 설계가 간단하여 개발 속도가 빠르고 작은 프로젝트에 알맞다.
* mvc2 : mvc1 에서 유지보수가 힘들다는 단점을 보완하기 위해 나온 모델 jsp는 뷰의 역할만 하고, 대신 컨트롤러 역할을 Servlet이 수행한다.
